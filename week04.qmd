...
- What differences do you notice between these three ways of creating vectors?
  How you input the range of 1 to 6 can present itself as different vectors. (1,2,3/1:3)
- Why might `typeof()` give different results?
  Because the numbers are different vectors. They maybe the same number but they are inputted a different way
...
- What happens when you mix integers and doubles in a vector?
  It defaults into a doubles integer
- What does the `L` suffix do?
  Make the vectors an integer
...
- What's the difference between `paste()`, `cat()`, and `glue()`?
  paste uses a separtor, cat is console without the quotes. glue is modern string interpolation
...
- Explain why `1 <- 1` causes an error
  assigning a number to an R value not comparison
- What's the difference between `=` and `==`?
  = is for assignment aswell, == is for the equal sign
...
- How are factors different from character vectors?
  Factors represent the group as categorical data instead of an unorganised mess.
...
John 1880 is higher prop of names while Linda 1947 has a higher count
